  ...
  findTables: (): HTMLElement[] => {
    ...
    codeBlocks.forEach(block => {
      if (block instanceof HTMLElement && !processedElements.has(block)) {
        ...
        if (Array.from(processedElements).some(processed => processed.contains(block))) {
          return;
        }
        ...
        if (text.includes('|') && text.split('\n').length >= 3) {
          ...
          if (tableLines.length >= 2) {
            ...
            logger.debug('Added code block table with', tableLines.length, 'lines');
          }
        }
      }
    });
    ...
    divs.forEach(div => {
      if (div instanceof HTMLElement && !processedElements.has(div)) {
        ...
        if (div.id?.includes('tabxport') || div.className?.includes('tabxport')) {
          return;
        }
        
        // Skip if this element is inside an already processed element
        if (Array.from(processedElements).some(processed => 
          processed.contains(div) || div.contains(processed)
        )) {
          return;
        }
        ...
        if (text.includes('|') && text.length > 50 && text.length < 5000) {
          ...
          const tableLines = lines.filter(line => {
            ...
            return parts.length >= 3 && parts.some(part => part.trim().length > 0);
          });
          ...
          if (tableLines.length >= 3) {
            ...
            if (tableRatio > 0.7) { // At least 70% of content should be table
              tables.push(div);
              processedElements.add(div);
              logger.debug('Added div table with', tableLines.length, 'lines, ratio:', tableRatio.toFixed(2));
            }
          }
        }
      }
