...
export const deepseekDetector: PlatformDetector = {
  ...
  findTables: (): HTMLElement[] => {
    ...
    allHTMLTables.forEach((table, index) => {
      ...
      const isLargeTable =
        rect.height > 300 ||
        rect.width > 600 ||
        (table as HTMLTableElement).rows.length > 10
      const isVeryWideTable =
        rect.width > window.innerWidth * 0.8 ||
        rect.right > window.innerWidth - 60

      logger.debug(
        `Table ${index} is large: ${isLargeTable} (${rect.width}x${rect.height}, ${(table as HTMLTableElement).rows.length} rows)`
      )
      logger.debug(
        `Table ${index} is very wide: ${isVeryWideTable} (causes horizontal scroll)`
      )
      ...
      if (
        ...
        if (isLargeTable) {
          ...
          const isScrollable =
            htmlTable.scrollHeight > htmlTable.clientHeight ||
    ...
    logger.debug(
      `Found ${messageContainers.length} message containers in DeepSeek`
    )
      ...
      codeBlocks.forEach((block, blockIndex) => {
        ...
        if (
          ...
          if (tableLines.length >= 2) {
            logger.debug(
              `Found markdown table in code block ${blockIndex} of message ${containerIndex}`
            )
            ...
            if (validLines.length >= 2) {
              logger.debug(
                `Adding markdown table ${blockIndex} from message ${containerIndex}`
              )

              // Check if this is a large markdown table
              const rect = htmlBlock.getBoundingClientRect()
              const isLargeTable =
                rect.height > 300 || rect.width > 600 || tableLines.length > 10
              const isVeryWideTable =
                rect.width > window.innerWidth * 0.8 ||
                rect.right > window.innerWidth - 60
              ...
              if (isLargeTable) {
                ...
                logger.debug(
                  `Large markdown table detected in block ${blockIndex}`
                )

                if (isVeryWideTable) {
                  htmlBlock.setAttribute("data-tabxport-very-wide", "true")
                  logger.debug(
                    `Very wide markdown table detected in block ${blockIndex}`
                  )
                }
              }
      ...
      contentDivs.forEach((div, divIndex) => {
        ...
        if (children.length >= 2 && children.length <= 10) {
          ...
          if (hasValidStructure && !processedElements.has(htmlDiv)) {
            logger.debug(
              `Adding div table ${divIndex} from message ${containerIndex}`
            )

            // Check if this is a large div table
            const rect = htmlDiv.getBoundingClientRect()
            const isLargeTable =
              rect.height > 300 || rect.width > 600 || children.length > 6
            const isVeryWideTable =
              rect.width > window.innerWidth * 0.8 ||
              rect.right > window.innerWidth - 60
      ...
      textElements.forEach((element, elementIndex) => {
        ...
        if (!isInsideProcessed && text.includes("|")) {
          ...
          if (tableLines.length >= 2) {
            ...
            if (validLines.length >= 2) {
              logger.debug(
                `Adding text table element ${elementIndex} from message ${containerIndex}`
              )
  ...
  extractChatTitle: (): string => {
    ...
    const titleElement = document.querySelector(
      ".chat-title, .conversation-title, h1.title"
    )
    if (titleElement) {
