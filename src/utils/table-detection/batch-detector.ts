import {
  addBatchTables,
  createBatchDetectionResult,
  createTableDetectionResult,
  getBatchTablesCount
} from "./batch-manager"
    ...
    try {
      if (parser.canParse(element)) {
        ...
        if (parseResult && isValidTableData(parseResult)) {
          ...
          logger.debug(
            `Successfully parsed table with ${parseResult.headers.length} headers and ${parseResult.rows.length} rows`
          )
          return tableData
        }
      }
...
export const detectAllTables = async (): Promise<BatchTableDetectionResult> => {
  ...
  try {
    ...
    logger.debug(
      `Platform detector found ${elements.length} potential table elements`
    )

    // Add debugging info about found elements
    elements.forEach((element, index) => {
      logger.debug(
        `Element ${index}: ${element.tagName} with classes: "${element.className}"`
      )
      logger.debug(
        `Element ${index} content preview: "${(element.textContent || "").substring(0, 100)}..."`
      )
    })
    ...
    for (const [index, element] of elements.entries()) {
      try {
        ...
        if (tableData) {
          ...
          logger.debug(
            `Successfully parsed table ${index + 1}/${elements.length}: ${tableData.headers.length} headers, ${tableData.rows.length} rows`
          )
        } else {
          logger.debug(
            `Failed to parse table element ${index + 1}/${elements.length}`
          )
        }
      } catch (error) {
    ...
    // Create batch result
    const batchResult = createBatchDetectionResult(
      tableResults,
      source,
      chatTitle
    )
    ...
    logger.debug(
      `Batch detection complete: found ${batchResult.count} valid tables for source ${source}`
    )
    logger.debug(
      `Table results:`,
      tableResults.map((r) => ({
        id: r.data.id,
        headers: r.data.headers.length,
