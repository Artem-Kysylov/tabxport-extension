...
const isValidContent = (text: string): boolean => {
  ...
  const meaningfulContentRegex = /[\w\u4e00-\u9fff\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE0F}]/u;
...
export const geminiDetector: PlatformDetector = {
  ...
  findTables: (): HTMLElement[] => {
    ...
    console.log(`TabXport Gemini: Found ${allHTMLTables.length} total HTML tables on page`);
    ...
    allHTMLTables.forEach((table, index) => {
      ...
      if ((table as HTMLTableElement).rows.length > 0 && htmlTable.offsetParent !== null) {
        ...
        console.log(`TabXport Gemini: Adding HTML table ${index} directly (${(table as HTMLTableElement).rows.length} rows)`);
    ...
    console.log(`TabXport Gemini: Found ${responseContainers.length} response containers`);
    ...
    responseContainers.forEach((container, containerIndex) => {
      ...
      console.log(`TabXport Gemini: Processing response container ${containerIndex}`);
      ...
      console.log(`TabXport Gemini: Found ${codeBlocks.length} code blocks in response ${containerIndex}`);
      ...
      codeBlocks.forEach((block, blockIndex) => {
        ...
        if (text.includes('|') && text.includes('\n')) {
          ...
          if (tableLines.length >= 2) {
            ...
            console.log(`TabXport Gemini: Adding markdown table from code block ${blockIndex} in response ${containerIndex} (${tableLines.length} lines)`);
        ...
        if (children.length >= 2 && children.length <= 10) { // Reasonable number of columns
          const hasValidStructure = Array.from(children).some(child => {
            ...
            return text.length > 0 && 
                   ...
                   isValidContent(text); // Use helper function that supports emojis
          });
          
          if (hasValidStructure && !processedElements.has(htmlDiv)) {
            logger.debug(`Adding div table ${divIndex} from response ${containerIndex}`);
            console.log(`TabXport Gemini: Adding div table ${divIndex} from response ${containerIndex} (${children.length} children)`);
      ...
      textElements.forEach((element, elementIndex) => {
        ...
        if (!isInsideProcessed && text.includes('|')) {
          ...
          if (tableLines.length >= 2) {
            ...
            console.log(`TabXport Gemini: Adding text table from response ${containerIndex} (${tableLines.length} lines)`);
    ...
    console.log(`TabXport Gemini: Final result - found ${elements.length} table elements`);
    return elements;
  },
