...
const isValidContent = (text: string): boolean => {
  ...
  const emojiRegex =
    /[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F900}-\u{1F9FF}]|[\u{1FA70}-\u{1FAFF}]|[\u{FE0F}]/u
...
export const validationUtils: ValidationUtils = {
  ...
  isValidTableData: (headers: string[], rows: string[][]): boolean => {
    ...
    const validHeaders = headers.filter((header, index) => {
      ...
      if (!isValid) {
        console.debug(
          `TabXport Validation: Invalid header ${index}: "${header}"`
        )
      }
    ...
    const hasValidRows = rows.every((row, rowIndex) => {
      ...
      if (!isValid) {
        console.debug(
          `TabXport Validation: Row ${rowIndex} has ${row.length} columns, expected ${expectedColumns}`
        )
      }
      return isValid
    })
    ...
    const validRowsCount = rows.filter((row, rowIndex) => {
      const hasValidData = row.some((cell, cellIndex) => {
        ...
        if (!isValid && cell.length > 0) {
          console.debug(
            `TabXport Validation: Row ${rowIndex}, Cell ${cellIndex} has invalid content: "${cell}"`
          )
        }
        return isValid
      })
    ...
    if (validRowsCount === 0) {
      console.debug(
        "TabXport Validation: Failed - no rows with meaningful data"
      )
    ...
    console.debug(
      `TabXport Validation: Success - ${headers.length} headers, ${validRowsCount}/${rows.length} valid rows`
    )
    return true
  },
