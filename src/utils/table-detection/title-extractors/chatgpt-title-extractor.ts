...
export const chatGPTTitleExtractor: TitleExtractor = {
  extractTitle: (): string => {
    ...
    for (const selector of headerSelectors) {
      ...
      if (element) {
        const text = domUtils.getTextContent(element as HTMLElement);
        if (text && text.length > 0 && !text.toLowerCase().includes('chatgpt')) {
    ...
    const firstMessage = document.querySelector('.text-base:first-child, [data-message-author="user"]:first-child');
    if (firstMessage) {
