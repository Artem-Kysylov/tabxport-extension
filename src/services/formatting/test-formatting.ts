import { FormattingUtils, TableFormatterService } from "./index"
...
async function runManualTests() {
  ...
  Object.entries(testData).forEach(([name, data]) => {
    ...
    console.log(
      `  Needs formatting: ${FormattingUtils.needsFormatting(data.headers, data.rows)}`
    )
  })
  ...
  for (const [name, data] of Object.entries(testData)) {
    console.log(`\nFormatting ${name}...`)
    try {
      const result = await FormattingUtils.quickFormat(
        data.headers,
        data.rows,
        "chatgpt"
      )
      ...
      console.log(
        `  Headers: ${result.headers.length} -> ${result.originalHeaders.length}`
      )
      console.log(
        `  Rows: ${result.rows.length} -> ${result.originalRows.length}`
      )
    } catch (error) {
  ...
  const platforms: Array<"chatgpt" | "claude" | "gemini" | "deepseek"> = [
    "chatgpt",
    "claude",
    "gemini",
    "deepseek"
...
export const examples = {
  ...
  platformSpecific: async () => {
    ...
    return await TableFormatterService.formatTable(
      headers,
      rows,
      options,
      "chatgpt"
