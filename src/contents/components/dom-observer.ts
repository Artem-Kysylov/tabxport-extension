import {
  detectAllTables,
  getBatchState
} from "../../utils/table-detection/batch-detector"
import {
  extractTableData,
  findAllTables,
  isValidTable
} from "../../utils/table-detector"
...
const getTargetContainers = (): HTMLElement[] => {
  ...
  // Claude
    ...
    containers.push(
      ...Array.from(
        document.querySelectorAll(
          '.chat-messages, .message-container, [class*="claude-"]'
        )
      ).filter((el): el is HTMLElement => el instanceof HTMLElement)
    )
  }
  // Gemini
    ...
    containers.push(
      ...Array.from(
        document.querySelectorAll(
          'mat-card, .message-container, [class*="gemini-"]'
        )
      ).filter((el): el is HTMLElement => el instanceof HTMLElement)
    )
  }
  // DeepSeek
    ...
    containers.push(
      ...Array.from(
        document.querySelectorAll(
          '.chat-container, .message-list, [class*="deepseek-"]'
        )
      ).filter((el): el is HTMLElement => el instanceof HTMLElement)
    )
  }
  ...
  if (containers.length === 0) {
    containers.push(
      ...Array.from(
        document.querySelectorAll("main, .main-content, .chat-container")
      ).filter((el): el is HTMLElement => el instanceof HTMLElement)
    )
  }
  ...
  for (const node of Array.from(mutation.addedNodes)) {
    if (node instanceof HTMLElement) {
      ...
      if (
        ...
        console.log(
          "TabXport: 🔍 Detected new table-like element:",
          node.tagName
        )
        return true
      }
      ...
      if (node.tagName === "DIV") {
        ...
        if (
          node.classList.contains("text-input-field") ||
...
export const scanAndProcessTables = async (): Promise<void> => {
  try {
    ...
    console.log(
      `TabXport: Current buttons: ${currentButtonCount}, Found tables: ${tableCount}, Valid batch tables: ${validBatchTablesCount}`
    )
    ...
    const hasValidationMismatch =
      Math.abs(currentButtonCount - validBatchTablesCount) > 1 // ИЗМЕНЕНО: более мягкий порог
    ...
    if (isFirstLoad) {
      ...
      console.log(
        "TabXport: Significant change detected - performing full validation"
      )
    } else if (
      hasValidationMismatch &&
      validBatchTablesCount > currentButtonCount
    ) {
      ...
      for (const [index, table] of tables.entries()) {
        if (!addedButtons.has(table)) {
          try {
            console.log(
              `TabXport: Processing new table ${index + 1}/${tables.length}`
            )
            const isValid = await isValidTable(table)
            if (isValid) {
              console.log(
                `TabXport: Adding export button to new table ${index}`
              )
            ...
            console.error(
              `TabXport: Error processing new table ${index}:`,
              error
            )
          }
        }
    ...
    console.log(
      `TabXport: Starting table validation for ${tables.length} tables`
    )
    ...
    for (let index = 0; index < tables.length; index++) {
      ...
      if (addedButtons.has(table)) {
        ...
        if (existingButton && document.contains(existingButton)) {
          console.log(
            `TabXport: Table ${index} already has valid button, skipping`
          )
          continue
        }
      }
      ...
      try {
        console.log(
          `TabXport: ===== Checking NEW table ${index + 1}/${tables.length} =====`
        )
        console.log(
          `TabXport: Table ${index} element:`,
          table.tagName,
          table.className || "no-class"
        )
        console.log(
          `TabXport: Table ${index} text preview:`,
          table.textContent?.substring(0, 150)
        )

        const isValid = await isValidTable(table)
        console.log(
          `TabXport: Table ${index} validation result: ${isValid ? "VALID" : "INVALID"}`
        )
    ...
    console.log(
      `TabXport: Validation complete. Active buttons: ${addedButtons.size}`
    )
    setTimeout(() => {
      console.log(
        `TabXport: Final scan results - Active buttons: ${addedButtons.size}`
      )
...
export const setupMutationObserver = (): void => {
  ...
  console.log(
    "TabXport: Setting up observers for containers:",
  ...
  observer = new MutationObserver((mutations) => {
    ...
    if (now - lastScanTime < 5000) {
      // УВЕЛИЧЕНО: минимальный интервал до 5 секунд
    ...
    console.log(
      `TabXport: 🔄 Found ${relevantMutations.length} relevant changes (${mutations.length} total mutations)`
    )
  ...
  if (mainContainer) {
    ...
    console.log(
      "TabXport: Observing main container:",
      mainContainer.tagName,
  ...
  setInterval(() => {
    ...
    if (mainContainer && observer) {
      ...
      console.log(
        "TabXport: Updated observation for main container:",
        mainContainer.tagName,
        mainContainer.className
      )
    }
