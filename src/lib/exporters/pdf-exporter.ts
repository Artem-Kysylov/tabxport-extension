...
const addPDFHeader = (doc: jsPDF, tableData: TableData): number => {
  ...
  const sourceTitle = tableData.source.charAt(0).toUpperCase() + tableData.source.slice(1);
...
export const exportToPDF = async (
  ...
  try {
    ...
    const tableConfig = {
      ...
      headStyles: {
        fillColor: [27, 147, 88], // Брендовый зеленый цвет #1B9358
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred during PDF export',
    };
  }
}; 