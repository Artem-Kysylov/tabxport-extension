...
export const chatGPTDetector: PlatformDetector = {
  ...
  findTables: (): HTMLElement[] => {
    ...
    let messageContainers = document.querySelectorAll('[data-message-author-role="assistant"]');
    console.log(`TabXport ChatGPT: Found ${messageContainers.length} messages with data-message-author-role="assistant"`);
    ...
    if (messageContainers.length === 0) {
      ...
      for (const selector of fallbackSelectors) {
        try {
          ...
          console.log(`TabXport ChatGPT: Trying selector "${selector}" - found ${found.length} elements`);
          if (found.length > 0) {
            messageContainers = found;
            console.log(`TabXport ChatGPT: Using fallback selector: ${selector}`);
            break;
          }
        } catch (error) {
    ...
    if (messageContainers.length === 0) {
      console.log('TabXport ChatGPT: No message containers found, searching entire page for tables...');
      
      // Look for any tables on the page
      const allTables = document.querySelectorAll('table');
      console.log(`TabXport ChatGPT: Found ${allTables.length} HTML tables on entire page`);
      
      allTables.forEach((table, index) => {
        if (table.rows.length > 0 && !processedElements.has(table as HTMLElement)) {
          console.log(`TabXport ChatGPT: Adding HTML table ${index} from page scan (${table.rows.length} rows)`);
      ...
      console.log(`TabXport ChatGPT: Found ${allCodeBlocks.length} code blocks on entire page`);
      ...
      allCodeBlocks.forEach((block, index) => {
        ...
        if (text.includes('|') && text.includes('\n')) {
          ...
          if (tableLines.length >= 2) {
            console.log(`TabXport ChatGPT: Adding markdown table from code block ${index} (${tableLines.length} lines)`);
      ...
      console.log(`TabXport ChatGPT: Page scan complete - found ${elements.length} table elements`);
      return elements;
    }
    
    logger.debug(`Found ${messageContainers.length} assistant messages`);
    console.log(`TabXport ChatGPT: Found ${messageContainers.length} assistant messages`);
    ...
    messageContainers.forEach((container, containerIndex) => {
      ...
      console.log(`TabXport ChatGPT: Found ${htmlTables.length} HTML tables in message ${containerIndex}`);
      
      htmlTables.forEach(table => {
        if (table.rows.length > 0 && !processedElements.has(table as HTMLElement)) {
          logger.debug(`Adding HTML table from message ${containerIndex}`);
          console.log(`TabXport ChatGPT: Adding HTML table from message ${containerIndex} (${table.rows.length} rows)`);
          elements.push(table as HTMLElement);
          processedElements.add(table as HTMLElement);
        }
      });
      ...
      console.log(`TabXport ChatGPT: Found ${codeBlocks.length} code blocks in message ${containerIndex}`);
      ...
      codeBlocks.forEach(block => {
        ...
        if (text.includes('|') && text.includes('\n')) {
          ...
          if (tableLines.length >= 2) {
            ...
            console.log(`TabXport ChatGPT: Adding markdown table from code block in message ${containerIndex} (${tableLines.length} lines)`);
      ...
      const textElements = messageElement.querySelectorAll('.markdown p, .markdown div, p, div');
      textElements.forEach(element => {
        ...
        if (!isInsideProcessed && text.includes('|')) {
          ...
          if (tableLines.length >= 2) {
            ...
            console.log(`TabXport ChatGPT: Adding text table from message ${containerIndex} (${tableLines.length} lines)`);
            // Note: Removed attribute setting to avoid React conflicts
    ...
    console.log(`TabXport ChatGPT: Final result - found ${elements.length} table elements`);
    return elements;
  },

  extractChatTitle: (): string => {
    // Try to find the chat title in the header
    const titleElement = document.querySelector('[class*="chat-title"], [class*="conversation-title"], h1, h2');
    if (titleElement) {
