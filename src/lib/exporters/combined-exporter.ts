...
import { Document, Packer, Paragraph, Table, TableRow, TableCell, TextRun, WidthType, BorderStyle, PageBreak } from 'docx';
...
export const exportCombinedCSV = async (
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred during combined CSV export',
    };
  }
};
...
const generateSectionTitle = (tableData: TableData, index: number): string => {
  ...
  const sourceName = tableData.source.charAt(0).toUpperCase() + tableData.source.slice(1);
 ...
 */
const createSectionTitle = (title: string, isFirst: boolean = false): Paragraph => {
...
export const exportCombinedDOCX = async (
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred during combined DOCX export',
    };
  }
};
...
const addTableToPDF = (
  ...
  const tableConfig = {
    ...
    theme: 'grid' as const,
    headStyles: {
      fillColor: [27, 147, 88], // Brand green
...
export const exportCombinedPDF = async (
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred during combined PDF export',
    };
  }
};
