...
export const divTableParser: TableParser = {
  canParse: (element: HTMLElement): boolean => {
    ...
    const hasTableRole = element.querySelector('[role="table"], .table, [class*="table"]') !== null;
    if (hasTableRole) {
      return true;
    }
  ...
  parse: (element: HTMLElement): { headers: string[]; rows: string[][] } | null => {
    ...
    const tableDiv = element.querySelector('[role="table"], .table, [class*="table"]');
    if (tableDiv) {
      ...
      dataRows.forEach((row, rowIndex) => {
        ...
        if (rowData.length > 0) {
          rows.push(rowData);
          logger.debug(`Parsed row ${rowIndex + 1} with ${rowData.length} cells`);
        }
      });
    }
    ...
    if (headers.length === 0 && rows.length === 0) {
      ...
      if (rowCandidates.length >= 2) {
        ...
        if (consistentRows.length >= 2) {
          ...
          logger.debug('Found', headers.length, 'headers from repeated structure');
    ...
    logger.debug('Div table parsing complete - Headers:', sanitizedData.headers.length, 'Data rows:', sanitizedData.rows.length);
