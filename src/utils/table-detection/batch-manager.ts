...
export const cleanupBatchTables = (): void => {
  ...
  if (removedCount > 0) {
    logger.debug(`Cleanup complete: removed ${removedCount} invalid tables, ${batchState.tables.size} remain`);
  }
...
export const clearAllBatchTables = (): void => {
  logger.debug(`Clearing all ${batchState.tables.size} tables from batch collection`);
 ...
 */
export const getBatchTablesDebugInfo = (): { count: number; tableIds: string[]; lastCleanup: number } => {
  return {
