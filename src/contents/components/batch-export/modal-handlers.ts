 ...
 */
export const updateProgressWithMessage = (modalState: BatchModalState, current: number, total: number, message?: string): void => {
 ...
 */
export const updateProgress = (modalState: BatchModalState, current: number, total: number): void => {
 ...
 */
export const handleTableSelection = (modalState: BatchModalState, tableId: string, selected: boolean, updateModalContentFn: () => void): void => {
  if (selected) {
 ...
 */
export const handleSelectAll = (modalState: BatchModalState, selectAll: boolean, updateModalContentFn: () => void): void => {
 ...
 */
export const handleCustomNameInput = (modalState: BatchModalState, tableId: string, customName: string): void => {
  if (customName.trim()) {
 ...
 */
export const updateModalContent = (modalState: BatchModalState, attachEventListenersFn: () => void): void => {
 ...
 */
export const handleBatchExport = async (modalState: BatchModalState, hideModalFn: () => void): Promise<void> => {
  ...
  if (modalState.config.exportMode === 'combined') {
    try {
      ...
      if (selectedTables.length > COMBINED_EXPORT_LIMITS.maxTables) {
        ...
        updateProgressWithMessage(modalState, 0, selectedTables.length, `❌ ${errorMessage}`);
      ...
      updateProgressWithMessage(modalState, 0, 1, `🔄 Combining ${selectedTables.length} tables into single file...`);
      ...
      if (result.success && result.downloadUrl) {
        ...
        updateProgressWithMessage(modalState, 1, 1, `✅ Combined file downloaded: ${result.filename}`);
        ...
        updateProgressWithMessage(modalState, 0, 1, `❌ Export failed: ${result.error}`);
      ...
      updateProgressWithMessage(modalState, 0, 1, `💥 Critical error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  for (let i = 0; i < selectedTables.length; i++) {
    ...
    try {
      ...
      updateProgressWithMessage(modalState, exportedCount, selectedTables.length, `Exporting table ${tableNumber}/${selectedTables.length}...`);
  ...
  if (modalState.config.exportMode === 'zip' && exportResults.length > 0) {
    try {
      ...
      updateProgressWithMessage(modalState, exportedCount, selectedTables.length, '📦 Creating ZIP archive...');
  ...
  updateProgressWithMessage(modalState, exportedCount, selectedTables.length, finalMessage);
